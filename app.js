...
//  this.id should be the same as the id we pass it. Same for the description and the value.
var budgetController = (function() {
    ...
    // Method: calcPercentage. We need to store our percentage somewhere so we do it in the Expense property above.
...
// Put methods in a prototype property of the expenses/income for the function constructors, So all objects created through them will inherit these methods.
// So they are not attached to each individual object but instead its object will then inherit it from the prototype.
...
// identified by their id. To delete a item from one of the 2 arrays is we need to know if we are talking about an expense or an income. So we need to setup
// the parameters here.
...
// Public calculate methods to calculate the expense percentage for each of the expense objects that are stored in the expenses array. 
    ...
    // Loop over all of our expenses for each of our objects. (function(cur)) is a callBack function which usually returns something. We return the result
    // of getPercentage method.
...
// Quick Win: 2 columns, 2 current accounts:
    ...
    var formatNumber = function(num, type) {
       ...
       num = num.toFixed(2); // 2 decimal numbers rule and storing it in the num variable.
        ...
        int = numSplit[0];
        // add a comma if we are into the thousands: if more than 3 numbers then we are in the thousands. Sub string - This method will return part of a string
        // that we want.
        if (int.length > 3) {
...
// element of the Array. 0 is the first element. Use the focus method.
        ...
        displayMonth: function() {
            ...
            months = ['Jnauary', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'Novemeber', 'December'];
            month = now.getMonth();
...
// We need a function to update the Budget Controller. This whole updatBudget function is called each time the user enter a new item into the UI
    ...
    // Updating the Percentages: everytime we add or delete an item. The % of each income that each expense represents. Creating new function so we can call
    // it later in our ctrlDeleteItem function similar to the updateBudget function. CALL this function in our other 2 functions.
    ...
    var ctrlAddItem = function() {
            ...
            // We want it not to be NaN, so its a number. Then we pass the input.value. Incase if the input value is 0, so add greater than operator.
            if (input.description !== "" && !isNaN(input.value) && input.value > 0) {

                // 2. Add the item to the budget controller. 
                // item method accepts 3 arguments which are the type, description and value, that we get. Using the input variable and the addItem method 
                // we create a new item we store it and we rtturn it and store it in this newItem variable - our new object which we going to pass to our
                // addListItem method. If its NOT a number it will return True. If it IS a number, it will return False. But we want the opposit so we use
                // the NOT opportator. We use the AND oporator as we want them to happen at the sametime.
